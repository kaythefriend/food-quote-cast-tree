...
function Canvas({casts,onSelect}:{casts:Cast[];onSelect:(c:Cast)=>void}){
 ...
 useEffect(()=>{if(!root||!ref.current)return;const r=positions.get(root.hash);if(!r)return;const vw=ref.current.clientWidth;const vh=ref.current.clientHeight;setPan({x:vw/2-(r.x+86)*zoom,y:Math.max(72,vh*.08)-r.y*zoom})},[root,positions,zoom])
 const onWheel=(e:React.WheelEvent)=>{e.preventDefault();setZoom(z=>Math.min(1.8,Math.max(.45,z*(e.deltaY<0?1.1:.9))))}
 const down=(e:React.PointerEvent)=>{if((e.target as HTMLElement).closest('.castCard'))return;drag.current={x:e.clientX,y:e.clientY,px:pan.x,py:pan.y,active:true};e.currentTarget.setPointerCapture(e.pointerId)}
 ...
 return <div ref={ref} className="treeViewport" onWheel={onWheel} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up}>
   <div className="treeStage" style={{width:w,height:h,left:0,top:0,transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`}}>
   ...
   <div className="zoomControls"><button onClick={()=>setZoom(z=>Math.min(1.8,z*1.15))}>+</button><button onClick={()=>setZoom(z=>Math.max(.45,z*.87))}>−</button><button onClick={reset}>Reset</button></div>
 </div>
}
